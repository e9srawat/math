...
def solver(numgrid, n: int):
    ...
    str_grid = [
        i.split(" ")
        for i in numgrid.split(
            "\n",
        )
    ]
    grid = [[int(j) for j in i] for i in str_grid]
    product = max(
        horizontal(grid, n), vertical(grid, n), diag_lr(grid, n), diag_rl(grid, n)
    )
    return product
...
def horizontal(grid, n):
    ...
    for i in grid:
        ...
        for j in range(n, (len(i) + 1)):
            product = max(product, math.prod(i[count:j]))
            count += 1
    return product
...
def diag_lr(grid, n):
    """
    Return the greatest product of n adjacent numbers
    in the diagonal left-right direction
    """
    product = 0
    for i in range(len(grid) - (n - 1)):
...
def diag_rl(grid, n):
    """
    Return the greatest product of n adjacent numbers
    in the diagonal right-left direction
    """
    product = 0
    for i in range(len(grid) - (n - 1)):
